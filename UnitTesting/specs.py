...
def count_sets(cards):
    ...
    """
    #check if there are exactly 12 cards
    if len(cards) != 12:
    ...
    #check if one or more cards has a character other than 0, 1 or 2
    for i in range (12):
        for j in range(4):
    ...
    num = 0
    for item in list(sets):
        if is_set(item[0], item[1], item[2]) == True:
            num += 1
    return num
...
def is_set(a, b, c):
    ...
    """
    #check each digit if it satisfy the condition to be a set
    for i in range(4):
